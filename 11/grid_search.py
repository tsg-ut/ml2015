...
if __name__ == "__main__":
    ...
    params1 = [5, 15, 25, 35, 45, 55, 65, 75, 85, 100]
    params2 = np.linspace(1e-0, 1e-4, 10)

    pv1, pv2 = np.meshgrid(params1, params2)

    scores = np.zeros(pv1.shape)

    for i in range(pv1.shape[0]):
        for j in range(pv1.shape[1]):
            # 進捗
            print "\x1b[4D{:>2}%".format(i*10+j),
            sys.stdout.flush()

            # Perceptronで教師データを学習する
            nn = Perceptron(nb_nodes=[pv1[i][j]], beta=pv2[i][j], eta=2.5e-2)
            nn.fit(train_x, train_y, pretraining=True)

            # 推定
            pred_y = [nn.predict(x) for x in test_x]
            scores[i][j] = metrics.accuracy_score(pred_y, test_y)

    print "\x1b[4D\x1b[K",

    plt.xlabel("number of nodes")
    plt.ylabel("beta")
    cont = plt.contourf(pv1, pv2, scores, [0, 0.5, 0.6, 0.7, 0.8, 0.9], cmap=plt.cm.get_cmap("winter"))
    plt.colorbar(cont)
    plt.show()
