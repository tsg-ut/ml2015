    ...
    def fit(self, x, y, pretraining=False):
        ...
        for i in range(int(np.ceil(nb_data / self.nb_batch))):
            ...
            if i < np.ceil(nb_data / self.nb_batch)-1:
                self.update(X[i*self.nb_batch:(i+1)*self.nb_batch].T, Y[i*self.nb_batch:(i+1)*self.nb_batch], pretraining=pretraining)
            else:
                self.update(X[i*self.nb_batch:nb_data].T, Y[i*self.nb_batch:nb_data], pretraining=pretraining)
    def update(self, x, t, pretraining=False):
        global e
        # 逆伝播計算
