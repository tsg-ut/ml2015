...
class Perceptron:
    ...
    def pretrain(self, x):
        ...
        for i in range(len(self.w)):
            ae = AutoEncoder(n_dim=self.dims[i+1], eta=self.eta, beta=self.b, r=self.r)
            ae.fit(x)
            self.w[i] = ae.weight()
            if i < len(self.w) - 1:
...
class AutoEncoder(Perceptron):
    def __init__(self, n_dim, eta=1e-2, beta=1e-2, r=1e-2):
        Perceptron.__init__(self, nb_nodes=[n_dim], eta=eta, beta=beta, r=r)
...
if __name__ == "__main__":
    ...
    nn = Perceptron(nb_nodes=[48, 24], beta=6.5e-2, r=5e-3, eta=9e-3)
